...
const LandingPage = () => {
    ...
    const socialBtnStyle = (hoverColor) => ({
        ...
        boxShadow: isDarkMode ? '0 4px 12px rgba(0, 0, 0, 0.1)' : '0 4px 12px rgba(15, 23, 42, 0.05)'
    });

    if (loading) return <div style={{ color: theme.textMain, textAlign: 'center', padding: '100px', backgroundColor: theme.bg }}>Going back to home...</div>;
        ...
        <div style={{ 
            ...
            <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 40px', borderBottom: `1px solid ${theme.border}`, backdropFilter: 'blur(10px)', position: 'relative', zIndex: 100 }}>
                ...
                <div onClick={() => setMobileMenuOpen(!mobileMenuOpen)} style={{ display: 'flex', flexDirection: 'column', gap: '5px', cursor: 'pointer', padding: '5px' }} className="hamburger-menu">
                    <div style={{ width: '24px', height: '2px', backgroundColor: theme.textMain, transition: '0.3s', transform: mobileMenuOpen ? 'rotate(45deg) translate(5px, 5px)' : 'none' }}></div>
                    <div style={{ width: '24px', height: '2px', backgroundColor: theme.textMain, transition: '0.3s', opacity: mobileMenuOpen ? 0 : 1 }}></div>
                    <div style={{ width: '24px', height: '2px', backgroundColor: theme.textMain, transition: '0.3s', transform: mobileMenuOpen ? 'rotate(-45deg) translate(5px, -5px)' : 'none' }}></div>
                </div>

                <nav style={{ display: mobileMenuOpen ? 'flex' : 'none', flexDirection: 'column', position: 'absolute', top: '100%', left: 0, right: 0, backgroundColor: theme.bg, padding: '20px', gap: '12px', borderBottom: `1px solid ${theme.border}` }} className="mobile-nav-links">
                    ...
                    <button style={{ ...navBtnStyle, width: '100%', background: theme.accent, color: '#fff' }} onClick={toggleTheme}>{isDarkMode ? '☀️ Light Mode' : '🌙 Dark Mode'}</button>
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }} className="desktop-nav-links">
                    ...
                    <button style={{ ...navBtnStyle, padding: '6px 12px', fontSize: '16px' }} onClick={toggleTheme} title={isDarkMode ? "Switch to Light Mode" : "Switch to Dark Mode"}>{isDarkMode ? '☀️' : '🌙'}</button>
                ...
                {totalFoldersExist === 0 && !selectedFolder ? (
                    <div style={{ display: 'grid', placeItems: 'center', height: '55vh', width: '100%' }}>
                        <div style={{ textAlign: 'center' }}>
                            ...
                            <h2 style={{ fontSize: '32px', fontWeight: '900', color: theme.textMain, margin: '0 0 12px 0', letterSpacing: '-0.5px' }}>You have no postss</h2>
                        ...
                        {!selectedFolder ? (
                            <>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                                    <h3 style={{ fontSize: '24px', fontWeight: '800', margin: 0, color: theme.textMain }}>All of your postss</h3>
                                            ...
                                            <div key={locationName} onClick={() => setSelectedFolder(locationName)} style={{ width: '100%', height: '380px', backgroundColor: theme.cardBg, border: `1px solid ${theme.border}`, borderRadius: '24px', overflow: 'hidden', cursor: 'pointer', position: 'relative', boxShadow: isDarkMode ? 'none' : '0 10px 30px rgba(0,0,0,0.06)' }}>
                                                ...
                                                <div style={{ 
                                                    ...
                                                    background: isDarkMode 
                                                        ? 'linear-gradient(to top, rgba(9, 13, 22, 0.95) 20%, rgba(9, 13, 22, 0.6) 70%, transparent 100%)' 
                                                        : 'linear-gradient(to top, rgba(255, 255, 255, 0.95) 20%, rgba(255, 255, 255, 0.6) 70%, transparent 100%)', 
                                                    display: 'flex', 
                                                    justifyContent: 'space-between', 
                                                    alignItems: 'center' 
                                                }}>
                                                    <div>
                                                        <h2 style={{ margin: 0, fontSize: '30px', fontWeight: '800', color: theme.textMain }}>📍 {locationName}</h2>
                                                        <p style={{ margin: '6px 0 0 0', color: theme.textMuted, fontSize: '13px', fontWeight: '600', maxWidth: '700px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>📝 Summary: {currentFolderSummary}</p>
                                                    </div>
                                                    <span style={{ backgroundColor: theme.accent, color: '#fff', padding: '8px 20px', borderRadius: '30px', fontSize: '13px', fontWeight: '700' }} className="desktop-only">{trips[locationName].length} Photos Locked</span>
                                                </div>
                                            </div>
                                        );
                                    })}
                            ...
                            <>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                                        <button onClick={() => setSelectedFolder(null)} style={{ padding: '10px 20px', background: theme.glassBtn, border: `1px solid ${theme.border}`, borderRadius: '12px', color: theme.textMain, cursor: 'pointer' }}>← Return to Grid</button>
                                        <h3 style={{ fontSize: '24px', fontWeight: '800', margin: 0, color: theme.textMain }}>Exploration stream: <span style={{ color: '#38bdf8' }}>{selectedFolder}</span></h3>
                                    </div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(440px, 1fr))', gap: '30px', marginBottom: '40px' }} className="folder-grid">
                                    {trips[selectedFolder].map((item) => {
                                        ...
                                        return (
                                            <div key={item._id} style={{ backgroundColor: theme.cardBg, border: `1px solid ${theme.border}`, borderRadius: '20px', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: isDarkMode ? '0 12px 32px rgba(0,0,0,0.3)' : '0 12px 32px rgba(15,23,42,0.06)', position: 'relative' }}>
                                                ...
                                                <div onClick={() => setFullscreenImage(item.image)} style={{ width: '100%', height: '300px', backgroundColor: isDarkMode ? '#05070c' : '#f1f5f9', display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'zoom-in', overflow: 'hidden' }}>
                                                ...
                                                <div style={{ padding: '22px', display: 'flex', flexDirection: 'column', gap: '10px', flexGrow: 1, justifyContent: 'space-between' }}>
                                                    <div>
                                                        <h4 style={{ fontSize: '18px', fontWeight: '800', margin: '0 0 6px 0', color: theme.textMain, letterSpacing: '-0.2px' }}>{displayTitle}</h4>
                                                        {displaySummary ? (
                                                            <p style={{ color: theme.textMain, fontSize: '13.5px', margin: '8px 0 0 0', lineHeight: '1.5', backgroundColor: theme.glassBtn, padding: '12px', borderRadius: '8px', border: `1px solid ${theme.border}` }}>{displaySummary}</p>
                                                        ) : (
            ...
            <footer style={{ borderTop: `1px solid ${theme.border}`, padding: '40px 20px', backgroundColor: isDarkMode ? 'rgba(4, 7, 13, 0.7)' : '#f1f5f9', backdropFilter: 'blur(12px)', marginTop: 'auto' }}>
            ...
            {showMainUploadModal && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(4, 7, 13, 0.85)', display: 'grid', placeItems: 'center', zIndex: 9999, backdropFilter: 'blur(8px)' }}>
                    <div style={{ backgroundColor: theme.modalBg, border: `1px solid ${theme.border}`, borderRadius: '24px', padding: '30px', width: '90%', maxWidth: '520px', boxSizing: 'border-box', position: 'relative' }}>
                        ...
                        <h3 style={{ fontSize: '20px', fontWeight: '800', margin: '0 0 4px 0', color: theme.textMain }}>Initialize New Discovery Core</h3>
                        <form onSubmit={handleMainFormSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginTop: '16px' }}>
                            <input type="text" placeholder="Folder Geographic Name (e.g. Bali Trip)" value={mainFolder} onChange={(e) => setMainFolder(e.target.value)} required style={{ width: '100%', padding: '12px', backgroundColor: theme.inputBg, border: `1px solid ${theme.border}`, borderRadius: '8px', color: theme.textMain, fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
                            ...
                            <input type="text" placeholder="Primary Image Title / Heading" value={mainTitle} onChange={(e) => setMainTitle(e.target.value)} required style={{ width: '100%', padding: '12px', backgroundColor: theme.inputBg, border: `1px solid ${theme.border}`, borderRadius: '8px', color: theme.textMain, fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
                            <textarea placeholder="Narrative description text parameters..." value={mainDescription} onChange={(e) => setMainDescription(e.target.value)} required rows="3" style={{ width: '100%', padding: '12px', backgroundColor: theme.inputBg, border: `1px solid ${theme.border}`, borderRadius: '8px', color: theme.textMain, fontSize: '14px', outline: 'none', resize: 'none', boxSizing: 'border-box' }} />
            ...
            {showModal && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(4, 7, 13, 0.85)', display: 'grid', placeItems: 'center', zIndex: 9999, backdropFilter: 'blur(8px)' }}>
                    <div style={{ backgroundColor: theme.modalBg, border: `1px solid ${theme.border}`, borderRadius: '24px', padding: '30px', width: '90%', maxWidth: '500px', boxSizing: 'border-box', position: 'relative' }}>
                        ...
                        <h3 style={{ fontSize: '18px', fontWeight: '700', margin: '0 0 20px 0', color: theme.textMain }}>Add Asset to {selectedFolder}</h3>
                        <form onSubmit={handleModalSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                            <input type="text" placeholder="Title" value={modalTitle} onChange={(e) => setModalTitle(e.target.value)} required style={{ width: '100%', padding: '12px', backgroundColor: theme.inputBg, border: `1px solid ${theme.border}`, borderRadius: '8px', color: theme.textMain, fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
                            <textarea placeholder="Summary description..." value={modalDescription} onChange={(e) => setModalDescription(e.target.value)} required rows="3" style={{ width: '100%', padding: '12px', backgroundColor: theme.inputBg, border: `1px solid ${theme.border}`, borderRadius: '8px', color: theme.textMain, fontSize: '14px', outline: 'none', resize: 'none', boxSizing: 'border-box' }} />
                            <div style={{ border: `1px dashed ${theme.accent}`, borderRadius: '8px', padding: '16px', textAlign: 'center', cursor: 'pointer' }}>
                                ...
                                <label htmlFor="modal-upload" style={{ cursor: 'pointer', fontSize: '13px', color: theme.accent, fontWeight: '700' }}>{modalImageFile ? `✓ ${modalImageFile.name}` : '📂 CHOOSE BINARY IMAGE'}</label>
                            </div>
